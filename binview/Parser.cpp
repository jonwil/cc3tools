...
bool elParser::ReadGranule(bsBitstream& IS, elGranule& Gr)
{
    ...
    // Read some fields in
    Gr.Version = (unsigned char)(IS.ReadBits(2));
    Gr.SampleRateIndex = (unsigned char)(IS.ReadBits(2));
    Gr.ChannelMode = (unsigned char)(IS.ReadBits(2));
    Gr.ModeExtension = (unsigned char)(IS.ReadBits(2));
    Gr.Index = (unsigned char)(IS.ReadBit());
